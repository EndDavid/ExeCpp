...
2.  It compiles `your_file.cpp` into an executable (e.g., `your_file` or `your_file.exe`) using g++ with author's default flags `-Wall -Wextra -std=gnu++23`.
...
- The compilation flags (`-Wall -Wextra -std=gnu++23`) are hardcoded in the source. You can modify the `_CUSTOM_ARGS` macro in `execpp.c` to suit your needs before compiling the tool.
